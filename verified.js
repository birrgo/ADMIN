...
// 1. FIX: Import ONLY the initialized db instance from your local config
import { db } from "./db.js";

// 2. FIX: Import ALL database utility functions from the exact same Firebase CDN link
import { 
    ref, 
    get, 
    set, 
...
if (otpForm) {
    otpForm.addEventListener('submit', async (e) => {
        ...
        if (!recipientEmail) {
            showToast("Please enter a recipient email address.", "red");
            return;
        }
        ...
        try {
            // Check 1 - Prevent dispatch if this email is already verified in otps
            const sanitizedEmailKey = sanitizeEmail(recipientEmail);
            ...
            if (otpVerifySnapshot.exists() && otpVerifySnapshot.val().verified === true) {
                showToast("This email is verified before", "red");
                if (dispatchBtn) dispatchBtn.disabled = false;
                return;
            }

            // Check 2 - Prevent dispatch if the email is registered to an existing user
            const usersRef = ref(db, 'users');
            const emailQuery = query(usersRef, orderByChild('emailAddress'), equalTo(recipientEmail.toLowerCase()));
            const emailSnapshot = await get(emailQuery);

            if (emailSnapshot.exists()) {
                showToast("An account already exists with this email address.", "red");
                if (dispatchBtn) dispatchBtn.disabled = false;
                return;
            }
